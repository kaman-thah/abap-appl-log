...
*"* do not include other source files here!!!
  PUBLIC SECTION.
    ...
    CLASS-DATA o_current TYPE REF TO zcl_appl_log .
    ...
    CLASS-METHODS create
      IMPORTING
        ...
        !id_extnumber      TYPE c
      RETURNING
        VALUE(ro_appl_log) TYPE REF TO zcl_appl_log .
    ...
    CLASS-METHODS save_all
      IMPORTING
        iv_alt_con TYPE abap_bool OPTIONAL .
    METHODS add
      IMPORTING
    ...
    METHODS add_bapi_order_return
      IMPORTING
        !is_msg TYPE bapi_order_return .
...
CLASS zcl_appl_log IMPLEMENTATION.
  ...
  METHOD add_free_text.
    ...
    CALL FUNCTION 'BAL_LOG_MSG_ADD_FREE_TEXT'
      ...
      EXCEPTIONS
        OTHERS         = 0.
    INSERT ls_msg_handle INTO TABLE t_msg.
  ...
  METHOD add_merrdat.
    ...
    ls_msg-msgty = is_merrdat-msgty.
    ls_msg-msgid = is_merrdat-msgid.
    ls_msg-msgno = is_merrdat-msgno.
    ls_msg-msgv1 = is_merrdat-msgv1.
    ls_msg-msgv2 = is_merrdat-msgv2.
    ls_msg-msgv3 = is_merrdat-msgv3.
    ls_msg-msgv4 = is_merrdat-msgv4.
  ...
  METHOD display.
    ...
    IF ls_profile-end_col < 100.
      ls_profile-end_col = 100.
    ENDIF.
  ...
  METHOD get_messages_as_text.
    ...
    LOOP AT lt_log_handles INTO l_log_h.
      ...
      IF sy-subrc IS INITIAL.
        IF i_filter = '*' OR i_filter = space.
          LOOP AT et_msg INTO ls_msg.
  ...
  METHOD save.
    ...
    DATA:
      ...
      lv_alt_con        TYPE boolean,
      lv_alt_con_commit TYPE boolean.
    IF me->saveable = abap_true.
      INSERT me->log_hndl INTO TABLE lt_log_handles.
      IF iv_alt_con = abap_true.
        lv_alt_con = abap_true.
        lv_alt_con_commit = abap_true.
      ENDIF.
      CALL FUNCTION 'BAL_DB_SAVE'
        EXPORTING
          i_t_log_handle       = lt_log_handles
          i_2th_connection     = lv_alt_con
          i_2th_connect_commit = lv_alt_con_commit
        IMPORTING
